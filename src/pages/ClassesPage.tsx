...
export default function ClassesPage() {
  ...
  const handleSaveClass = async () => {
    try {
      ...
      // Auto-set subject to name if empty
      if (!classData.subject) classData.subject = classData.name;

      if (editingClass) {
        await classService.updateClass(editingClass.id, classData, batchIds);
        toast.success('Class updated successfully');
      } else {
        // Default to first branch if "All Branches" is selected
        const effectiveBranchId = currentBranchId || contextBranches[0]?.id || null;
